...
from src.utils.indicators import (
    ...
    calculate_atr, calculate_sma, calculate_bollinger_bands
)
...
class BacktestEngine:
    ...
    def download_data(self) -> Optional[pd.DataFrame]:
        ...
        while current_time < end_time:
            try:
                ...
                print(f"   下载 {current_time.strftime('%Y-%m-%d %H:%M')} ~ {request_end.strftime('%Y-%m-%d %H:%M')} ...", end='')
        ...
        # 转换为DataFrame
        if all_klines:
            self.df = pd.DataFrame(all_klines, columns=[
    ...
    def calculate_indicators(self) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            print("✅ 指标计算完成")
            print(f"   包含指标: RSI, MACD, EMA, SMA, ATR, 布林带")
    ...
    def analyze_signals(self) -> Dict[str, Any]:
        ...
        print(f"   看涨K线: {analysis['bullish_candles']} ({100*analysis['bullish_candles']/analysis['total_candles']:.1f}%)")
        print(f"   看跌K线: {analysis['bearish_candles']} ({100*analysis['bearish_candles']/analysis['total_candles']:.1f}%)")
...
【数据统计】
总K线数: {analysis['total_candles']}
开盘至收盘: {self.df['open'].iloc[0] if self.df is not None else 0:.2f} → {self.df['close'].iloc[-1] if self.df is not None else 0:.2f}
价格涨跌: {backtest['price_change_percent']:+.2f}%
最大回撤: {backtest['max_drawdown_percent']:.2f}%

【K线分析】
看涨K线: {analysis['bullish_candles']} ({100*analysis['bullish_candles']/analysis['total_candles']:.1f}%)
看跌K线: {analysis['bearish_candles']} ({100*analysis['bearish_candles']/analysis['total_candles']:.1f}%)

【交易信号】
买入信号: {analysis['buy_signals']}
卖出信号: {analysis['sell_signals']}

【回测结果】
交易总数: {backtest['statistics'].get('total_trades', 0)}
胜率: {backtest['statistics'].get('win_rate', 0):.1f}%
总盈亏: {backtest['statistics'].get('total_pnl', 0):.2f} USDT
回测收益: {backtest['total_return_percent']:+.2f}%
...
"""
        ...
        if analysis['signals']:
            ...
            for signal in analysis['signals'][-10:]:
                report += f"  {signal['time']} - {signal['signal']:4} @ {signal['price']:.2f} (RSI={signal['rsi']:.1f})\n"
        ...
        if backtest['trades']:
            ...
            for trade in backtest['trades'][-5:]:
                ...
                return_str = f"+{trade['pnl_percent']:.2f}%" if trade['pnl_percent'] > 0 else f"{trade['pnl_percent']:.2f}%"
