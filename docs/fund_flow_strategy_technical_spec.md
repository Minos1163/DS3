...
## 当前主策略（2026-03-16 起）
- 交易标的：以 `config/trading_config_fund_flow.json` 为准，当前配置为 `BTCUSDT`、`ETHUSDT`、`BNBUSDT`、`SOLUSDT`、`XAGUSDT`、`LTCUSDT`、`DOGEUSDT`。
- 主周期：`1H`，只负责方向过滤。
- 入场周期：`15M`，只负责起爆共振。
- 趋势门：价格在 `1H EMA30` 上方只做多；价格在 `1H EMA30` 下方只做空；`1H EMA30` 走平则观望。
- 入场规则 A：`15M EMA10/EMA30 + MACD` 共振。
  - 多头：`EMA10` 上穿 `EMA30`，且 `MACD` 零轴上金叉或柱体翻红。
  - 空头：`EMA10` 下穿 `EMA30`，且 `MACD` 零轴下死叉或柱体翻绿放大。
- 入场规则 B：`15M EMA10/EMA30 + Bollinger` 共振。
  - 多头：价格突破布林上轨，且布林带开口扩大。
  - 空头：价格跌破布林下轨，且布林带开口扩大。
- 动态止损：多单跌破 `15M EMA30` 或 `15M MACD` 死叉止损；空单涨破 `15M EMA30` 或 `15M MACD` 在零轴下金叉止损；初始止损宽度严格控制在 `2%~5%`。
- 动态止盈：盈利进入 `5%~10%` 区间先减仓 `1/2`；剩余仓位多单不破 `15M EMA10` 持有、`15M MACD` 死叉平仓，空单不破 `15M EMA10` 持有、`15M MACD` 在零轴下金叉平仓；收盘有效跌破/涨破 `EMA10` 则全部离场。

## 旧链路状态
- `5m/3m` 资金流评分链路不再作为主开仓依据。
- 这些低周期字段只保留为辅助过滤、归因日志和 confidence 参考。
- AI Weight Router 不得覆盖 `1H/15M` 主决策链。
