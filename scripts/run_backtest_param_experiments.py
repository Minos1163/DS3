...
def run_experiment(name: str, stop_loss_pct: float = None):
    ...
    return {"name": name, "final_capital": bt.capital, "total_pnl": total_pnl, "trades": len(bt.trades)}
...
def main():
    ...
    for r in results:
        if r is None:
            continue
        print(
            f"{r['name']}: final_capital={r['final_capital']:.2f}, total_pnl={r['total_pnl']:.2f}, trades={r['trades']}"
        )
