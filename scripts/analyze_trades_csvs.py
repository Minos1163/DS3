...
def analyze_file(path: str):
    ...
    stats = df['pnl'].describe(percentiles=[.1, .25, .5, .75, .9]).to_dict()
        ...
        if 'confidence_buckets' in a:
            for b in a['confidence_buckets']:
                if b['count'] > 20 and b['mean'] > 0:
                    ...
                    rules.append(f"For {name}: require confidence >= {str(b['conf_bin']).split(',')[-1].strip(' ])') if b.get('conf_bin') else 'x'} (see buckets)")
                    break

    # if strategies have very low win rates, suggest adding momentum filter
    for a in analyses:
        if a['win_rate_pct'] < 20 and a['n'] > 50:
            rules.append(f"For {a['file']}: win_rate {a['win_rate_pct']:.1f}% — add momentum filter (e.g. MACD>signal or close>EMA50) or increase min ATR/stop-distance.")

    # if average hold time is very long, suggest max_hold
    for a in analyses:
        if a['avg_hold_minutes'] and a['avg_hold_minutes'] > 60 * 24:
            rules.append(f"For {a['file']}: average holding time {a['avg_hold_minutes']:.0f} min — consider max holding duration (e.g. 200 candles).")

    # general recommendations
    rules.append("General: add min_volume or min_atr threshold at signal time; require multi-indicator agreement (RSI/MACD/EMA); increase AI min_confidence to 0.7+ and validate by backtesting confidence buckets.")
    return rules
