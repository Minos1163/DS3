...
if dd_window is not None and len(dd_window):
    ...
    for i, row in dd_window.iterrows():
        print(
            f"{i}: {row['entry_time']} -> {row['exit_time']}, {row['direction']}, pnl={row['pnl']:.4f}, cumulative={row['cumulative_capital']:.4f}"
        )
...
for _, r in df.nsmallest(5, "pnl").iterrows():
    print(
        f"{r['entry_time']} {r['direction']} pnl={r['pnl']:.4f} pct={r['pnl_pct']:.4f} exit_reason={r['exit_reason']}"
    )
