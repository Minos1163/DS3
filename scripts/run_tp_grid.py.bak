import os
import sys
import csv
from datetime import datetime

# ensure project root
PROJECT_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
sys.path.insert(0, PROJECT_ROOT)

from backtest_15m30d_v2 import ConservativeBacktester

OUTPUT_DIR = 'logs'
os.makedirs(OUTPUT_DIR, exist_ok=True)


def run_experiment(tp):
    bt = ConservativeBacktester(initial_capital=100.0, leverage=10.0)
    bt.take_profit_pct = tp
    bt.stop_loss_pct = 0.006
    bt.cooldown_bars = 12
    bt.max_consecutive_losses = 2

    df = bt.load_data('data/SOLUSDT_15m_30d.csv')
    df = bt.calculate_indicators(df)
    bt.run_backtest(df)

    trades = bt.trades
    total_trades = len(trades)
    wins = len([t for t in trades if t['pnl'] > 0])
    win_rate = wins / total_trades * 100 if total_trades else 0
    total_pnl = sum(t['pnl'] for t in trades)
    final_capital = bt.capital

    import pandas as pd
    if trades:
        df_tr = pd.DataFrame(trades)
        df_tr['cumulative_capital'] = 100.0 + df_tr['pnl'].cumsum()
        df_tr['peak_capital'] = df_tr['cumulative_capital'].cummax()
        df_tr['drawdown'] = (df_tr['peak_capital'] - df_tr['cumulative_capital']) / df_tr['peak_capital']
        max_dd = df_tr['drawdown'].max() * 100
    else:
        max_dd = 0

    return {
        'take_profit_pct': tp,
        'final_capital': final_capital,
        'total_pnl': total_pnl,
        'max_drawdown': max_dd,
        'total_trades': total_trades,
        'win_rate': win_rate,
    }


def main():
    tps = [0.10, 0.12, 0.14]
    results = []
    for tp in tps:
        print(f"Running TP={tp*100:.0f}%...")
        res = run_experiment(tp)
        print(f" -> final {res['final_capital']:.2f} USDT, trades={res['total_trades']}, maxDD={res['max_drawdown']:.2f}%")
        results.append(res)

    ts = datetime.now().strftime('%Y%m%d_%H%M%S')
    out_file = os.path.join(OUTPUT_DIR, f'tp_grid_results_{ts}.csv')
    with open(out_file, 'w', newline='', encoding='utf-8') as f:
        writer = csv.DictWriter(f, fieldnames=['take_profit_pct','final_capital','total_pnl','max_drawdown','total_trades','win_rate'])
        writer.writeheader()
        for r in results:
            writer.writerow(r)

    print('\nDone. Saved to', out_file)

if __name__ == '__main__':
    main()
