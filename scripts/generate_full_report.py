...
def generate_html(report_md, top_csv, cross_csv, btc_csv, plots, out_html):
    ...
    parts.append('<link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css">')
    ...
    for p in plots:
        ...
        parts.append(f'<img src="{data_uri}" style="max-width:100%;height:auto;border:1px solid #ccc;padding:6px;background:#fff"/>')
        parts.append('</div>')
...
def main():
    ...
    if not cross_csv:
        # fallback
        cross_list = sorted([p for p in os.listdir('logs') if p.startswith('cross_top') or p.startswith('cross_validation')])
