...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    parser.add_argument("--in", dest="infile", default=r"d:\\AIDCA\\AIBOT\\logs\\dca_dashboard.csv", help="输入 dca_dashboard CSV 文件路径")
    parser.add_argument("--out", dest="outfile", default=r"d:\\AIDCA\\AIBOT\\logs\\worst_periods_by_side.csv", help="输出 CSV 路径")
    parser.add_argument("--start", dest="start", type=parse_dt, default=datetime(2026, 2, 5, 18, 0, 0), help="开始时间（ISO 或 yyyy-mm-dd HH:MM:SS）")
    parser.add_argument("--end", dest="end", type=parse_dt, default=datetime(2026, 2, 5, 21, 24, 0), help="结束时间（ISO 或 yyyy-mm-dd HH:MM:SS）")
    parser.add_argument("--float-places", dest="float_places", type=int, default=6, help="输出浮点数固定小数位数（默认 6）")
    parser.add_argument("--missing-num", dest="missing_num", type=float, default=-1.0, help="数值字段缺失时的占位符（默认 -1）")
    ...
    with open(out_path, 'w', newline='', encoding='utf-8') as f:
        w = csv.writer(f)
        w.writerow(['side','symbol','start_ts','end_ts','min_pnl','min_ts','duration_s','entry_price','mark_price','dca_count'])
        ...
        for side, wobj in sorted(final):
            ...
            entry_price_out = fmt.format(entry_price_val) if isinstance(entry_price_val, (int, float)) else str(entry_price_val)
            mark_price_out = fmt.format(mark_price_val) if isinstance(mark_price_val, (int, float)) else str(mark_price_val)
            min_pnl_out = fmt.format(wobj['min_pnl']) if isinstance(wobj['min_pnl'], (int, float)) else str(wobj['min_pnl'])
            w.writerow([
                side,
                wobj['symbol'],
    ...
    for side, wobj in sorted(final):
        ...
        print('  PERIOD', wobj['start_ts'].isoformat(), '->', wobj['end_ts'].isoformat(), 'duration_s=', wobj['duration_s'])
