"""
15m 30天优化回测 V2 - 做多为主策略
核心改进:
1. 以做多为主（顺应上涨趋势）
2. 严格止损（1.5%）
3. 合理止盈（5%）
4. 降低仓位（15%）
5. 禁止连续亏损开仓
"""
import os
import sys
import pandas as pd
import numpy as np
from datetime import datetime
from typing import Dict, List, Any, Optional

sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))


class ConservativeBacktester:
    """保守回测器 - 做多为主"""
    
    def __init__(self, initial_capital: float = 10000.0, leverage: float = 10.0):
        self.initial_capital = initial_capital
        self.leverage = leverage
        self.capital = initial_capital
        self.peak_capital = initial_capital
        
        self.position = None
        self.entry_price = 0.0
        self.position_size = 0.0
        self.entry_time = None
        
        self.trades = []
        self.consecutive_losses = 0  # 连续亏损次数
        
        # 保守参数
        self.position_percent = 0.12     # 12%仓位（更保守）
        self.stop_loss_pct = 0.012       # 1.2%止损（更严格）
        self.take_profit_pct = 0.045     # 4.5%止盈
        
        # RSI参数 - 放宽范围增加交易机会
        self.rsi_oversold = 40  # 从35提高到40
        self.rsi_overbought = 60  # 从70降低到60
        
        # 成交量过滤
        self.volume_quantile = 0.40  # 降低成交量要求
        self.volume_window = 60
        
        # 时段过滤
        self.allowed_hours = set(range(6, 22))
        
        # 风控
        self.max_consecutive_losses = 3
        self.max_drawdown_percent = 0.20

    def load_data(self, filepath: str) -> Optional[pd.DataFrame]:
        print(f"\n{'='*80}")
        print(f"📂 加载数据: {filepath}")
        print(f"{'='*80}")
        
        if not os.path.exists(filepath):
            print(f"❌ 文件不存在")
            return None
        
        df = pd.read_csv(filepath, index_col='timestamp', parse_dates=True)
        print(f"✅ 数据加载成功 ({len(df)}点, {(df.index[-1] - df.index[0]).days}天)")
        return df

    def calculate_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        print(f"\n{'='*80}")
        print(f"📊 计算技术指标")
        print(f"{'='*80}")
        
        df = df.copy()
        
        # RSI
        delta = df['close'].diff()
        gain = delta.where(delta > 0, 0)
        loss = -delta.where(delta < 0, 0)
        avg_gain = gain.rolling(window=14).mean()
        avg_loss = loss.rolling(window=14).mean()
        rs = avg_gain / avg_loss
        df['rsi'] = 100 - (100 / (1 + rs))
        
        # EMA
        df['ema_5'] = df['close'].ewm(span=5, adjust=False).mean()
        df['ema_20'] = df['close'].ewm(span=20, adjust=False).mean()
        df['ema_50'] = df['close'].ewm(span=50, adjust=False).mean()
        
        # 布林带
        df['bb_middle'] = df['close'].rolling(window=20).mean()
        bb_std = df['close'].rolling(window=20).std()
        df['bb_upper'] = df['bb_middle'] + (bb_std * 2)
        df['bb_lower'] = df['bb_middle'] - (bb_std * 2)
        
        # 成交量
        df['volume_quantile'] = df['volume'].rolling(window=self.volume_window).apply(
            lambda x: pd.Series(x).rank(pct=True).iloc[-1], raw=False
        )
        
        print(f"✅ 指标计算完成")
        return df

    def check_entry_signal(self, row) -> Optional[str]:
        """检查入场信号 - 6个指标满足3个即可"""
        # 风控：连续亏损过多则暂停
        if self.consecutive_losses >= self.max_consecutive_losses:
            return None
        
        # 回撤保护
        current_drawdown = (self.peak_capital - self.capital) / self.peak_capital
        if current_drawdown >= self.max_drawdown_percent:
            return None
        
        close = row['close']
        rsi = row['rsi']
        ema_5 = row['ema_5']
        ema_20 = row['ema_20']
        ema_50 = row['ema_50']
        bb_lower = row['bb_lower']
        bb_upper = row['bb_upper']
        bb_middle = row['bb_middle']
        volume_quantile = row['volume_quantile']
        
        # 基础过滤
        if pd.isna(rsi) or pd.isna(ema_20) or pd.isna(volume_quantile):
            return None
        
        # 时段过滤
        hour = row.name.hour
        if hour not in self.allowed_hours:
            return None
        
        # 去掉强制成交量过滤，改为在指标中使用
        
        # 做多信号 - 6个指标，满足3个即可
        long_conditions = [
            rsi < self.rsi_oversold,              # 1. RSI超卖
            close <= bb_lower * 1.08,              # 2. 价格接近下轨
            ema_5 > ema_20,                        # 3. 短期EMA上穿中期
            ema_20 > ema_50,                       # 4. 中期趋势向上  
            close < bb_middle,                     # 5. 价格在中轨下方
            volume_quantile >= 0.45                # 6. 成交量过滤
        ]
        long_score = sum(long_conditions)
        
        if long_score >= 3:
            return 'LONG'
        
        # 做空信号 - 6个指标，满足3个即可
        short_conditions = [
            rsi > self.rsi_overbought,             # 1. RSI超买
            close >= bb_upper * 0.92,              # 2. 价格接近上轨
            ema_5 < ema_20,                        # 3. 短期EMA下穿中期
            ema_20 < ema_50,                       # 4. 中期趋势向下
            close > bb_middle,                     # 5. 价格在中轨上方
            volume_quantile >= 0.45                # 6. 成交量过滤
        ]
        short_score = sum(short_conditions)
        
        if short_score >= 3:
            return 'SHORT'
        
        return None

    def execute_trade(self, row, signal: str):
        """执行交易"""
        if self.position is not None:
            return
        
        price = row['close']
        timestamp = row.name
        
        position_value = self.capital * self.position_percent * self.leverage
        self.position = signal
        self.entry_price = price
        self.position_size = position_value / price
        self.entry_time = timestamp
        
        print(f"\n{'='*60}")
        print(f"📈 开仓 {signal}")
        print(f"   时间: {timestamp}")
        print(f"   价格: {price:.4f}")
        print(f"   仓位: {position_value:.2f} USDT ({self.position_percent*100}%)")
        print(f"   止损: {self.stop_loss_pct*100}% | 止盈: {self.take_profit_pct*100}%")
        print(f"   连续亏损: {self.consecutive_losses}次")
        print(f"{'='*60}")

    def check_exit(self, row):
        """检查出场条件 - 严格止损"""
        if self.position is None:
            return
        
        price = row['close']
        timestamp = row.name
        
        # 计算收益率
        if self.position == 'LONG':
            pnl_pct = (price - self.entry_price) / self.entry_price
        else:  # SHORT
            pnl_pct = (self.entry_price - price) / self.entry_price
        
        exit_reason = None
        
        # 止损（优先级最高）
        if pnl_pct <= -self.stop_loss_pct:
            exit_reason = 'STOP_LOSS'
        
        # 止盈
        elif pnl_pct >= self.take_profit_pct:
            exit_reason = 'TAKE_PROFIT'
        
        if exit_reason:
            self.close_position(price, timestamp, exit_reason, pnl_pct)

    def close_position(self, price: float, timestamp, reason: str, pnl_pct: float):
        """平仓"""
        position_value = self.entry_price * self.position_size
        # position_value 已包含杠杆（在开仓时乘过），此处不应再次乘以 self.leverage
        pnl = pnl_pct * position_value
        
        self.capital += pnl
        self.peak_capital = max(self.peak_capital, self.capital)
        
        # 更新连续亏损计数
        if pnl < 0:
            self.consecutive_losses += 1
        else:
            self.consecutive_losses = 0  # 重置
        
        trade = {
            'entry_time': self.entry_time,
            'exit_time': timestamp,
            'direction': self.position,
            'entry_price': self.entry_price,
            'exit_price': price,
            'position_size': self.position_size,
            'pnl': pnl,
            'pnl_pct': pnl_pct * 100,
            'exit_reason': reason,
            'capital_after': self.capital,
            'consecutive_losses': self.consecutive_losses
        }
        self.trades.append(trade)
        
        print(f"\n{'='*60}")
        print(f"📉 平仓 {self.position}")
        print(f"   时间: {timestamp}")
        print(f"   原因: {reason}")
        print(f"   收益率: {pnl_pct*100:.2f}%")
        print(f"   盈亏: {pnl:+.2f} USDT")
        print(f"   余额: {self.capital:.2f} USDT")
        if pnl < 0:
            print(f"   ⚠️ 连续亏损: {self.consecutive_losses}次")
        print(f"{'='*60}")
        
        self.position = None

    def run_backtest(self, df: pd.DataFrame):
        """运行回测"""
        print(f"\n{'='*80}")
        print(f"🚀 开始回测（保守做多策略）")
        print(f"{'='*80}")
        print(f"初始资金: {self.initial_capital:.2f} USDT")
        print(f"仓位: {self.position_percent*100}% | 杠杆: {self.leverage}x")
        print(f"止损: {self.stop_loss_pct*100}% | 止盈: {self.take_profit_pct*100}%")
        
        for idx, row in df.iterrows():
            if self.position is not None:
                self.check_exit(row)
            
            if self.position is None:
                signal = self.check_entry_signal(row)
                if signal:
                    self.execute_trade(row, signal)
        
        # 强制平仓
        if self.position is not None:
            last_row = df.iloc[-1]
            pnl_pct = ((last_row['close'] - self.entry_price) / self.entry_price if self.position == 'LONG' 
                      else (self.entry_price - last_row['close']) / self.entry_price)
            self.close_position(last_row['close'], last_row.name, 'END_OF_DATA', pnl_pct)

    def analyze_results(self):
        """分析结果"""
        print(f"\n{'='*80}")
        print(f"📊 回测结果分析")
        print(f"{'='*80}")
        
        if not self.trades:
            print("❌ 无交易记录")
            return
        
        df_trades = pd.DataFrame(self.trades)
        
        # 基本统计
        total_trades = len(df_trades)
        winning_trades = len(df_trades[df_trades['pnl'] > 0])
        losing_trades = len(df_trades[df_trades['pnl'] < 0])
        win_rate = winning_trades / total_trades * 100 if total_trades > 0 else 0
        
        total_pnl = df_trades['pnl'].sum()
        total_return = (self.capital - self.initial_capital) / self.initial_capital * 100
        
        # 回撤
        df_trades['cumulative_capital'] = self.initial_capital + df_trades['pnl'].cumsum()
        df_trades['peak_capital'] = df_trades['cumulative_capital'].cummax()
        df_trades['drawdown'] = (df_trades['peak_capital'] - df_trades['cumulative_capital']) / df_trades['peak_capital']
        max_drawdown = df_trades['drawdown'].max() * 100
        
        # 盈亏统计
        avg_win = df_trades[df_trades['pnl'] > 0]['pnl'].mean() if winning_trades > 0 else 0
        avg_loss = abs(df_trades[df_trades['pnl'] < 0]['pnl'].mean()) if losing_trades > 0 else 0
        profit_factor = avg_win / avg_loss if avg_loss > 0 else 0
        
        print(f"\n【总体表现】")
        print(f"初始资金: {self.initial_capital:.2f} USDT")
        print(f"最终资金: {self.capital:.2f} USDT")
        print(f"总收益: {total_pnl:+.2f} USDT ({total_return:+.2f}%)")
        print(f"最大回撤: {max_drawdown:.2f}%")
        
        print(f"\n【交易统计】")
        print(f"总交易: {total_trades} | 盈利: {winning_trades} | 亏损: {losing_trades}")
        print(f"胜率: {win_rate:.2f}%")
        print(f"盈亏比: {profit_factor:.2f}")
        print(f"平均盈利: {avg_win:.2f} USDT")
        print(f"平均亏损: {avg_loss:.2f} USDT")
        
        # 方向统计
        print(f"\n【方向统计】")
        direction_counts = df_trades['direction'].value_counts()
        for direction, count in direction_counts.items():
            print(f"{direction}: {count}次")
        
        # 保存结果
        self.save_results(df_trades)

    def save_results(self, df_trades: pd.DataFrame):
        """保存结果"""
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        
        trades_file = f"logs/conservative_15m30d_trades_{timestamp}.csv"
        df_trades.to_csv(trades_file, index=False)
        print(f"\n✅ 交易记录: {trades_file}")
        
        summary_file = f"logs/conservative_15m30d_summary_{timestamp}.txt"
        with open(summary_file, 'w', encoding='utf-8') as f:
            f.write(f"15m 30天保守做多策略\n")
            f.write(f"{'='*60}\n\n")
            f.write(f"初始资金: {self.initial_capital:.2f} USDT\n")
            f.write(f"最终资金: {self.capital:.2f} USDT\n")
            f.write(f"总收益: {self.capital - self.initial_capital:+.2f} USDT\n")
            f.write(f"总收益率: {(self.capital - self.initial_capital) / self.initial_capital * 100:+.2f}%\n")
            f.write(f"最大回撤: {df_trades['drawdown'].max() * 100:.2f}%\n")
            f.write(f"\n总交易: {len(df_trades)}\n")
            f.write(f"胜率: {len(df_trades[df_trades['pnl'] > 0]) / len(df_trades) * 100:.2f}%\n")
        
        print(f"✅ 摘要: {summary_file}")


def main():
    backtester = ConservativeBacktester(
        initial_capital=100.0,  # 100 USDT
        leverage=10.0
    )
    
    df = backtester.load_data("data/SOLUSDT_15m_30d.csv")
    if df is None:
        return
    
    df = backtester.calculate_indicators(df)
    backtester.run_backtest(df)
    backtester.analyze_results()
    
    print(f"\n{'='*80}")
    print(f"✅ 回测完成")
    print(f"{'='*80}")


if __name__ == '__main__':
    main()
